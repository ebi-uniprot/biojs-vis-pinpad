...
var addDatumTemp = function(info, cols, tr) {
    if (info.key_right === true) {
        cols.leftCol = tr.append('td');
        cols.rightCol = tr.append('td').text(info.key + ' ');
    } else {
        cols.leftCol = tr.append('td').text(info.key);
        cols.rightCol = tr.append('td');
    }
    if (info.key_style) {
        cols.leftCol.attr('style', info.key_style);
    }
    if (info.value_style) {
        cols.rightCol.attr('style', info.value_style);
    }
};
...
var addDatum = function(table, key, info) {
    ...
    if (key.indexOf('styled_') === 0) {
        ...
        addDatumTemp(info, cols, tr);
        addValue(cols.rightCol, info.value);
