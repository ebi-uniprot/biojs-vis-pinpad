...
var PinPadModel = function(ordering) {
    ...
    pinPadModel.addElement = function(element) {
        var category;
        var catIndex = -1;
        ...
        if (elem) {
            ...
            if (!category || (catIndex === -1)) {
                category = addCategory(pinPadModel, element.category, ordering);
    ...
    pinPadModel.removeElement = function(elId) {
        ...
        if ((catIndex !== -1) && (elIndex !== -1)) {
            ...
            var removedElement = pinPadModel.categories[catIndex].elements[elIndex];
            pinPadModel.categories[catIndex].removeElement(elIndex);
            if (pinPadModel.categories[catIndex].elements.length === 0) {
                elIndex = pinPadModel.categories.splice(catIndex, 1);
            }
            return {catIndex: catIndex, elIndex: elIndex, category: removedFromCategory, element: removedElement};
        }
        return {error: 'unknown'}
    };

    pinPadModel.removeCategory = function(catTitle) {
        var catIndex = -1;
